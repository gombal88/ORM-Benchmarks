...
class AdbInterface:
    ...
    def Uninstall(self, package_name, clear_data=True):
        """Uninstalls apk on device.
        Args:
          package_name: package name of app to uninstall on host
        Returns:
          output of uninstall command
        """
        shell_command = 'pm uninstall '
        if not clear_data:
            shell_command += '-k '
        shell_command += package_name
        return self.SendShellCommand(shell_command)

    def ClearAppData(self, package_name):
        """Clear application data.
        Args:
          package_name: package name of app to clear data on host
        Returns:
          output of clearing app data command
        """
        shell_command = 'pm clear '
        shell_command += package_name
        return self.SendShellCommand(shell_command)

    def StopAppProcess(self, package_name):
        """Stop application.
        Args:
          package_name: package name of app to stop on host
        Returns:
          output
        """
        shell_command = 'am force-stop '
        shell_command += package_name
        return self.SendShellCommand(shell_command)
